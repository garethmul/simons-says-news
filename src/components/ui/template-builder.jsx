...
import { VariableTagInput, VariableTag } from './variable-tag';
import { 
  Plus, 
  Save, 
  Play, 
...
export const TemplateBuilder = ({
  ...
  const validateTemplate = () => {
    ...
    const matches = templateData.prompt.matchAll(variablePattern);
    const unresolvedVars = [];
    
    for (const match of matches) {
      const varName = match[1].trim();
      if (!defaultVariables.find(v => v.name === varName)) {
        unresolvedVars.push(varName);
      }
    }

    if (unresolvedVars.length > 0) {
      errors.prompt = `Unresolved variables: ${unresolvedVars.join(', ')}`;
    }
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            <TabsContent value="settings" className="space-y-6">
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p className="text-sm">Input/Output schemas, UI configuration, etc.</p>
              </div>
            </TabsContent>
