...
-----END CERTIFICATE-----`,
  ...
  try {
    ...
    // 1. VERIFY PROMPT FIXES
    console.log('📋 1. VERIFYING PROMPT FIXES\n');
    ...
    console.log('✅ CURRENT PROMPT STATUS:');
    updatedPrompts.forEach(prompt => {
      ...
      if (prompt.notes) console.log(`  - Notes: ${prompt.notes}`);
      console.log('');
    });
    
    // 2. CLEAR PREVIOUS CONTENT
    console.log('🧹 2. CLEARING PREVIOUS CONTENT FOR FRESH TEST\n');
    ...
    if (finalContent.length === 8) {
      ...
      console.log('\n🔍 Still missing some content types. Need further investigation.');
    }
